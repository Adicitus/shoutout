BeforeAll {
    Set-ShoutOutDefaultLog { param($Record) $Record | Write-Host -ForegroundColor Gray  } -Global
}
...
Describe "LogHandler" {
        ...
        Context "File" {
            Context "LogFilePath" {
                ...
                It "ShouldWriteRecordToFile" {
                    ...
                    $msgType = "Info"
                    Set-ShoutOutDefaultLog -LogFilePath $testLogFile
                    shoutOut -Message $msg -MsgType $msgType -Quiet
        ...
        Context "ScriptBlock" {
            ...
            Context "Using Parameter 'Message'" {
                It 'Should receive the object used as message.' {
                    $out = @{}
                    $srcMsg = [guid]::NewGuid()
                    Set-ShoutOutRedirect -LogHandler { param($Message) $out.Msg = $Message } -MsgType MessageHandler -Global
            ...
            Context "Using Parameter 'Details'" {
                BeforeAll {
                    ...
                    $out = @{}
                    Set-ShoutOutRedirect -LogHandler { param($Details, $Record) $out.v = $Details; } -MsgType $MsgType -Global
                }
    ...
    Context "Scoping" {
        ...
        BeforeEach {
            ...
            Clear-ShoutOutLog -MessageType $msgType -Global
            Add-ShoutOutLog -LogHandler { param($Message) $out.Result += $Message } -MessageType $msgType -Global
        }

        It "Should propagate messages to the global scope" {
            $msgType = 'ScopeTest'
            $out = @{
                Result = ""
            }
            shoutOut 1 $msgType
            . {
                Add-ShoutOutLog $msgType { param($Message) $out.Result += $Message }
                . {
                    shoutOut 2 $msgType
                }
            }
            $out.Result | Should -Be '122'
        }

        It "Should remove log handler once we leave the scope" {
            $msgType = 'ScopeTest'
            $out = @{
                Result = ""
            }
